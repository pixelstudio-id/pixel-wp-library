...
add_action('admin_enqueue_scripts', '_px_add_highlighter_to_head_footer_code', 101);

/**
 * Add syntax highlighter for the Head & Footer code
 * 
 * @action admin_enqueue_scripts
 */
function _px_add_highlighter_to_head_footer_code() {
  if (isset($_GET['page']) && $_GET['page'] !== 'head-footer-code') { return; }

  $settings = wp_enqueue_code_editor(['type' => 'text/html']);
  if (!$settings) { return; }

  wp_localize_script('px-admin', 'pxCodemirrorSettings', $settings);
}
